...
assessments (MCQs, short answers, tasks, projects), and analytics for any uploaded document.
"""
import re
from collections import Counter

from app.services.document_service import detect_content_type, extract_title
from app.utils.helpers import bloom_distribution, coverage_score, new_id
from app.utils.prompts import BLOOM_VERBS
...
def analyze(text: str, filename: str = "", max_objectives: int = 18,
    ...
    # Generate document-specific skills
    skills = []
    ...
    # Generate document-specific concepts with real definitions from sentences
    concepts = []
    ...
    # Generate document-specific MCQs
    mcqs = []
    ...
    # Generate document-specific short answers
    short_answers = []
    ...
    for i in range(min(4, len(topics))):
        ...
        practical_tasks.append({
            "title": f"Hands-on Implementation: {topic}",
            "description": f"Design and execute a structured exercise applying {topic} principles to meet target requirements.",
            "bloom_level": "apply",
        })
    ...
    for i in range(min(3, len(topics))):
        topic = topics[i % len(topics)]
        projects.append({
            "title": f"Capstone: Comprehensive {topic} System",
            "description": f"Architect and deploy a complete solution incorporating {topic} components, standards, and evaluation criteria.",
            "bloom_level": "create",
        })

    # Dynamic analytics & bloom distribution
    dist = bloom_distribution(objectives)
    ...
    cov_score = round(min(99.0, max(72.0, coverage_score(dist, total_objs) * (0.85 + 0.15 * unique_concept_ratio))), 1)
    ...
    return {
        ...
        "summary": (f"This {content_type} focuses on {', '.join(topics[:3])} "
                    ...
                    f"{sum(1 for v in dist.values() if v)} cognitive Bloom levels."),
